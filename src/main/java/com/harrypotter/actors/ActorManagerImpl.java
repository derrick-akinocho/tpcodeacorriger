...
public class ActorManagerImpl implements ActorManager {
    ...
    public NameAge getTheOldestActor() throws SQLException {
        NameAge na = null;
        Statement st = con.createStatement();
        ResultSet rs = st.executeQuery("SELECT CONCAT(lastname,\" \", firstname) name, " +
                "round(DATEDIFF(CURDATE(), BIRTHDATE)/365) age FROM actor ORDER by age DESC LIMIT 1;");
        if (rs.next()) {
    ...
    public NameAge getTheYoungestActor() throws SQLException {
        ...
        Statement st = con.createStatement();
        ResultSet rs = st.executeQuery("SELECT CONCAT(lastname,\" \", firstname) name, " +
                "round(DATEDIFF(CURDATE(), BIRTHDATE)/365) age FROM actor ORDER by age LIMIT 1;");
        if (rs.next()) {
            na = new NameAge(rs.getString("name"), rs.getByte("age"));
        }
        return na;
