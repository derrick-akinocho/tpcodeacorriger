...
public class MovieManagerImpl implements MovieManager {
    ...
    public List<String> getPartDesc(String word) throws SQLException{
        ...
        PreparedStatement ps = con.prepareStatement(
                "SELECT SUBSTRING(description, LOCATE(?, description), LENGTH(description) - LOCATE(?, description) + 1) AS partdesc FROM movie");
        ps.setString(1, word);
        ps.setString(2, word);
        ResultSet rs = ps.executeQuery();
        while(rs.next()){
    ...
    public String getDistinctFirstnameOfDirectors() throws SQLException {
        ...
        ResultSet rs = st.executeQuery("SELECT DISTINCT SUBSTRING_INDEX(director, ' ', 1) AS firstnameDirector FROM movie");
        while(rs.next()){
