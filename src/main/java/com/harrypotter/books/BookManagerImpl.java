...
public class BookManagerImpl implements BookManager {
    ...
    public List<String> getPartOfBookDescriptions(int numberOfWords) throws SQLException {
        ...
        PreparedStatement ps = con.prepareStatement(
                "SELECT SUBSTRING_INDEX(description, ' ', ?) part FROM book");
    ...
    public int getSumOfLengthDescriptions() throws SQLException {
        int result = 0;
        Statement st = con.createStatement();
        ResultSet rs = st.executeQuery("SELECT SUM(LENGTH(description)) sumLength FROM book");
        if(rs.next()){
    ...
    public List<Book> getBooksWithOddID() throws SQLException {
        List<Book> books = new ArrayList<>();
        Statement st = con.createStatement();
        ResultSet rs = st.executeQuery("SELECT * FROM book WHERE id % 2 = 1");
        while(rs.next()){
            Book b = new Book(rs.getInt("ID"),
                    rs.getString("title"),
                    rs.getString("description"));
